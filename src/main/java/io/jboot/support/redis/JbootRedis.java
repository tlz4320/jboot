...
public interface JbootRedis {
     ...
     * 此方法用了修改 incr 等的值
     */
    String setWithoutSerialize(Object key, Object value);
     ...
     */
    String setex(Object key, int seconds, Object value);
     ...
     * @return
     */
    String getWithoutSerialize(Object key);
     ...
     */
    Long decrBy(Object key, long value);
     ...
     */
    Long expire(Object key, int seconds);
     ...
     */
    List hmget(Object key, Object... fields);
     ...
     * 当给定多个 key 参数时，按参数 key 的先后顺序依次检查各个列表，弹出第一个非空列表的头元素。
     */
    List blpop(Integer timeout, Object... keys);
     ...
     * 关于阻塞操作的更多信息，请查看 BLPOP 命令， BRPOP 除了弹出元素的位置和 BLPOP 不同之外，其他表现一致。
     */
    List brpop(Integer timeout, Object... keys);
     ...
     * 上下三个函数都改成List  已经没必要用Set了 既然是要保持顺序的
     */
     ...
     * @param channels
     */
    void subscribe(JedisPubSub listener, final String... channels);
     ...
     * @param channels
     */
    void subscribe(BinaryJedisPubSub binaryListener, final byte[]... channels);
     ...
     * @return
     */
    RedisScanResult<String> scan(String pattern, String cursor, int scanCount);
    ...
    void fieldSetFromBytesSet(Set<byte[]> data, Set<Object> result);

    byte[] valueToBytes(Object value);

    Object valueFromBytes(byte[] bytes);

    byte[][] valuesToBytesArray(Object... valuesArray);

    void valueSetFromBytesSet(Set<byte[]> data, Set<Object> result);
